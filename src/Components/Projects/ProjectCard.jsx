...
const ProjectCard = ({ slug, title, category, imgsrc, oneLiner, demo }) => {
  return (
    <div className="group relative flex flex-col w-full rounded-2xl overflow-hidden
      border border-ink/10 dark:border-base-line bg-paper-card dark:bg-base-elevated
      hover:border-accent/40 hover:shadow-node-light dark:hover:shadow-node
      ...
      <div className="relative overflow-hidden aspect-[16/10] bg-paper-sunk dark:bg-base-surface flex items-center justify-center">
        <img
      ...
      <div className="p-5 md:p-6 flex flex-col flex-1">
        ...
        <p className="text-sm leading-relaxed text-ink-muted dark:text-mist-muted mt-2 flex-1">
          {oneLiner}
        </p>

        <div className="flex items-center gap-4 mt-5 pt-4 border-t border-ink/10 dark:border-base-line">
          <Link
            to={`/project/${slug}`}
            className="flex items-center gap-1.5 text-sm font-medium text-accent dark:text-accent-soft
            hover:text-accent-strong dark:hover:text-white transition-colors duration-200"
          ...
          {/* <a
            ...
            hover:text-ink dark:hover:text-mist transition-colors duration-200 ml-auto"
          >
            Live Site <LuArrowUpRight size={13} />
          </a> */}
          {demo ? (
  <a
    href={demo}
  ...
  <span className="text-sm font-mono text-signal ml-auto">{status || 'Available soon'}</span>
)}
        </div>
      </div>
    </div>
